...
Get-Service | Where-Object {$_.Name -like 'MSSQLSERVER'}| ForEach-Object{
    ...
    $fullInstance = $SQLServer + "\$dirtyInstanceName"
    $instName = $SQLServer + "\$instName"
    ...
    $connectionString = "Server = $fullInstance,1433; Integrated Security = True;"
    ...
    try{
        $connection.Open()
        #Write-Host "OK connecting to $fullInstance!"
    }
    catch{
